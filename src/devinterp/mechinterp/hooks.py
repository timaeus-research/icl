...
class Hooked(nn.Module):
    def __init__(self, module: nn.Module):
        """Wraps a PyTorch module to cache its._output during forward pass.

        Attributes:
            _module (nn.Module): Original module.
            _forward (Callable): Original forward method.
           ._output: Cached._output of the last forward pass.
        """
        super().__init__()
        self._module = module
    ...
    def collect_cache(self):
        """Collects cached._outputs from the current module and its immediate children.

        Returns:
            dict: Nested dictionary of cached._outputs.
        """
        cache = {"": self._output}
    ...
    def forward(self, *args, **kwargs):
        """Runs the forward pass and caches the._output."""
        self._output = self._forward(self, *args, **kwargs)
        return self._output

    def run_with_cache(self, *args, **kwargs):
        """Runs the forward pass and collects cached._outputs.
...
class HookedList(nn.ModuleList):
    """Extension of PyTorch's ModuleList to support caching of._outputs."""

    def collect_cache(self):
        """Collects cached._outputs from the list of modules.
...
class HookedSequential(nn.Sequential):
    """Extension of PyTorch's Sequential to support caching of._outputs."""

    def collect_cache(self):
        """Collects cached._outputs from the sequence of modules.
